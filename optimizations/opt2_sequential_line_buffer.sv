 ...
 *
 * CoreMark op2: sequential same-line line buffer — capture on any SRAM hit (not only branch
 * targets), plus fill-write conflict invalidation. See
 * ibex_edited/plans/icache_planner_agent_coremark_op2.md.
 */
...
module ibex_icache import ibex_pkg::*; #(
  ...
  // Line buffer: suppresses tag/data SRAM reads on repeated fetches to the same line (after any hit)
  ...
  /////////////////////////////////////////
  // Line buffer (same-line read bypass) //
  /////////////////////////////////////////
  ...
  if (ICacheLineBuffer) begin : gen_line_buf
    ...
    logic [NUM_FB-1:0]         fill_line_buf_match;
    logic                      line_buf_fill_conflict;
    ...
    // Invalidate when a fill commits a RAM write to the same cache line as the buffer
    for (genvar fb = 0; fb < NUM_FB; fb++) begin : gen_fill_line_buf_match
      assign fill_line_buf_match[fb] = fill_addr_q[fb][ADDR_W-1:IC_LINE_W] == line_buf_tag_q;
    end
    assign line_buf_fill_conflict = line_buf_valid_q & |(fill_ram_arb & fill_line_buf_match);
    ...
    assign line_buf_inval = icache_inval_i | ~icache_enable_i |
                            (ecc_err_ic1 & line_buf_hit_ic1) | line_buf_fill_conflict;
