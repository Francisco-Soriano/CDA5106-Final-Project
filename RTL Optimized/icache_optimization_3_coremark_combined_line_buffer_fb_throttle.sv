 ...
 *
 * CoreMark combined power optimizations (see ibex_edited/plans/icache_planner_agent_coremark_op3.md):
 * (1) Same-line line buffer — sequential hits bypass tag/data SRAM reads.
 * (2) Fill-buffer lookup suppression — no SRAM read when a busy FB already covers the line.
 * (3) Prefetch depth throttling — lower FB fill threshold plus optional stall-aware suppression.
 */
...
module ibex_icache import ibex_pkg::*; #(
  ...
  // Same-line line buffer: suppress SRAM reads on repeated hits within the last-hit line
  parameter bit          ICacheLineBuffer = 1'b1,
  // Prefetch throttle: fire when fb_fill_level exceeds this (default 1: stricter than NUM_FB-2)
  parameter int unsigned ICacheFbThrottleThresh = 1,
  // When set, suppress speculative prefetch while fill-buffer output is valid but IF is stalled
  parameter bit          ICachePrefetchStallThrottle = 1'b1
) (
  // Clock and reset
  input  logic                           clk_i,
  input  logic                           rst_ni,
  ...
  output logic                           perf_ic_tag_read_o,
  output logic                           perf_ic_data_read_o,
  output logic                           perf_ic_tag_write_o,
  output logic                           perf_ic_data_write_o,
  output logic                           perf_ic_evict_o,
  output logic                           perf_ic_inval_tag_write_o
);
  ...
  logic [IC_INDEX_W-1:0]                  lookup_index_ic1;
  ...
  logic [IC_NUM_WAYS-1:0]                 plru_way_ic1;
  logic [IC_NUM_WAYS-1:0]                 repl_way_ic1;
  logic [IC_NUM_LINES-1:0]                plru_mru_way_q;
  ...
  logic [NUM_FB-1:0]                      fill_evict_d, fill_evict_q;
  ...
  // Fill buffer lookup suppression signals (Optimization 2)
  logic [NUM_FB-1:0]                      fill_addr_line_match;
  logic                                   fill_match_ic0;
  ...
  // Cache lookup (Optimization 3: tunable FB threshold + optional stall-aware suppression)
  assign lookup_throttle  = (fb_fill_level > ICacheFbThrottleThresh[$clog2(NUM_FB)-1:0]);

  assign lookup_stall_suppress = ICachePrefetchStallThrottle & data_valid & ~ready_i;

  assign lookup_req_ic0   = req_i & ~&fill_busy_q &
                            (branch_i | (~lookup_throttle & ~lookup_stall_suppress)) &
                            ~ecc_write_req;
  assign lookup_addr_ic0  = branch_i ? addr_i : prefetch_addr_q;
  assign lookup_index_ic0 = lookup_addr_ic0[IC_INDEX_HI:IC_LINE_W];

  // Fill buffer lookup suppression (Optimization 2): suppress SRAM reads when an
  // active fill buffer is already fetching the requested cache line.
  for (genvar fb = 0; fb < NUM_FB; fb++) begin : gen_fill_addr_match
    assign fill_addr_line_match[fb] = fill_busy_q[fb] &
        (fill_addr_q[fb][ADDR_W-1:IC_LINE_W] == lookup_addr_ic0[ADDR_W-1:IC_LINE_W]);
  end
  assign fill_match_ic0 = lookup_req_ic0 & (|fill_addr_line_match);
  ...
  // Suppress IC1 when SRAM was not read due to fill buffer match, but allow
  // line buffer hits through since the line buffer provides its own data path.
  assign lookup_actual_ic0 = lookup_grant_ic0 & icache_enable_i & ~inval_block_cache &
                             (line_buf_hit_ic0 | ~fill_match_ic0);
  ...
  // One-cycle RAM activity event pulses
  assign perf_ic_tag_read_o        = tag_req_ic0 & ~tag_write_ic0;
  assign perf_ic_data_read_o       = data_req_ic0 & ~data_write_ic0;
  assign perf_ic_tag_write_o       = tag_write_ic0;
  assign perf_ic_data_write_o      = data_write_ic0;
  assign perf_ic_inval_tag_write_o = inval_write_req;
  ...
  // 2-way PLRU victim selection from per-set MRU state.
  ...
  // Tag RAM is not read on line_buf hit; invalid-way info would be stale—use replacement way only
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      if (tag_hit_ic1) begin
        plru_mru_way_q[lookup_index_ic1] <= tag_match_ic1[1];
      end else begin
        plru_mru_way_q[lookup_index_ic1] <= sel_way_ic1[1];
      end
    end
  end
  ...
  if (ICacheECC) begin : gen_data_ecc_checking
    ...
    logic [IC_INDEX_W-1:0]      ecc_correction_index_q;
  ...
  for (genvar fb = 0; fb < NUM_FB; fb++) begin : gen_fbs
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
      if (!rst_ni) begin
        ...
        fill_evict_q[fb]    <= fill_evict_d[fb];
  ...
  // Evictions are counted when an allocation write commits into a valid victim line.
  always_comb begin
    perf_ic_evict_o = 1'b0;
    for (int i = 0; i < NUM_FB; i++) begin
      perf_ic_evict_o |= fill_ram_arb[i] & fill_evict_q[i];
    end
  end
  ...
  /////////////////////////////////////////
  // Line buffer (same-line hit bypass)   //
  /////////////////////////////////////////
  ...
  if (ICacheLineBuffer) begin : gen_line_buf
    ...
    // IC0 hit: qualify with cache-enabled path (avoids circ. dependency with lookup_actual_ic0)
    assign line_buf_hit_ic0 = line_buf_valid_q & lookup_grant_ic0 & icache_enable_i &
        ~inval_block_cache & (lookup_addr_ic0[ADDR_W-1:IC_LINE_W] == line_buf_tag_q);
    assign sram_lookup_ic0  = lookup_req_ic0 & ~line_buf_hit_ic0 & ~fill_match_ic0;
    ...
    assign line_buf_inval = branch_i | icache_inval_i | ~icache_enable_i |
        (ecc_err_ic1 & line_buf_hit_ic1) | fill_grant_ic0 | inval_write_req | ecc_write_req |
        ic_scr_key_req_o;
    ...
    // Bypass: SRAM sees the raw lookup request, suppressed by fill buffer match
    assign sram_lookup_ic0  = lookup_req_ic0 & ~fill_match_ic0;
  ...
  `ASSERT_INIT(icache_plru_2way_only, (IC_NUM_WAYS == 2))
