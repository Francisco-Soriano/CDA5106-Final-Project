...
module ibex_icache import ibex_pkg::*; #(
  ...
  // OPTIMIZATION 2 (icache_plan2_reduce_num_fb): Reduced from 4 → 2 to halve fill-buffer
  // register toggling and cut dynamic power.  FB_THRESHOLD drops to 0, so throttling
  // engages as soon as any one non-stale buffer is busy (more conservative but functionally
  // correct — counter never overflows because throttle prevents new allocation).
  // See RTL Optimization Proposals/icache_power_optimization2.md for full analysis.
  localparam int unsigned NUM_FB        = 2;   // reduced for power; minimum allowed is 2
  // Request throttling threshold (automatically becomes 0 with NUM_FB = 2)
