...
module ibex_icache import ibex_pkg::*; #(
  ...
  // Way-prediction signals (Optimization Plan 1)
  ...
  logic [IC_INDEX_W-1:0]                  lookup_index_ic1_q; // IC0 index pipelined to IC1 (global)
  ...
  // Way-prediction: predict which data RAM way to read in IC0.
  ...
  // One-hot select of predicted way for normal lookups;
  // ECC write and fill write paths are unchanged (all ways / fill way).
  ...
  if (ResetAll) begin : g_lookup_addr_ra
    ...
    always_ff @(posedge clk_i) begin
      if (lookup_grant_ic0) begin
        ...
        lookup_index_ic1   <= lookup_addr_ic0[IC_INDEX_HI:IC_LINE_W];
        fill_in_ic1        <= fill_alloc_sel;
        lookup_index_ic1_q <= lookup_index_ic0; // Way-prediction: pipe index to IC1
      end
    end
  end

  // Way-prediction: pipeline predicted way to IC1
  always_ff @(posedge clk_i) begin
    if (lookup_grant_ic0)
  ...
  // Suppress the hit so the fill buffer treats a misprediction as a miss
  assign tag_hit_ic1_masked = tag_hit_ic1 & ~way_mispredict_ic1;

  // Update hint on every hit (use unmasked tag_hit_ic1 so mispredictions
  // also update the hint to the correct way for the next access)
  ...
  if (ICacheECC) begin : gen_data_ecc_checking
    ...
    // Use masked tag_hit so that a way misprediction does not trigger a spurious
    // ECC data error on the unread (wrong) way
  ...
  for (genvar fb = 0; fb < NUM_FB; fb++) begin : gen_fbs
    ...
    // Record whether the request hit in the cache (use masked signal so mispredictions
    // are treated as misses and the fill buffer does not capture wrong data)
