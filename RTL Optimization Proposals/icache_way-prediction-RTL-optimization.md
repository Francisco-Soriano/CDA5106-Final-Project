...
### Step 2 — Register lookup index into IC1 (~line 340)

`lookup_addr_ic1` only stores upper tag bits (`ADDR_W-1:IC_INDEX_HI+1`), not the set index. Add `lookup_index_ic1_q` inside the existing pipeline register block:

```systemverilog
// existing (non-ResetAll variant shown; add same line to ResetAll variant):
...
// Update hint on every hit (correct or mispredicted — correct way is always in tag_match_ic1)
...
`**ecc_err_ic1` inside `gen_data_ecc_checking` (~line 451):**
